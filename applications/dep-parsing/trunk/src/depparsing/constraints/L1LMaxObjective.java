...
//			if (expectations==null) expectations = new double[](parent);
//			logLikelihood = 0;
...
//					SentenceChildParent scp = parent.edge2scp[constr][i];
//					if (scp.p < 0){
//						posteriors[scp.s].root[scp.c] -= ((double[])lambda).value[constr][i];
//					} else {
//						for(int v = 0; v < posteriors[scp.s].nontermMap.childValency; v++)
//							posteriors[scp.s].child[scp.c][scp.p][v] -= ((double[])lambda).value[constr][i];
//					}
//				}
//			}
//
...
//	}
//	
	public L1LMaxObjective(double[] initialLambda, L1Lmax parent, DepSentenceDist[] posteriors) {
...
//			cache.update();
//		}
//		double[] res = (double[]) cache.expectations.deepCopy();
//		projectGradient(res);
//		return res;
//	}
//
...
//		doLambdaProjection((double[]) lambda);
//	}
//
//
...
//		double[] grad = (double[]) getGradient();
//		double origObjective = getObjective();
//		double[] origLambda = (double[]) lambda.deepCopy();
//		double[] newLambda = (double[]) lambda.deepCopy();
...
// 		getObjective();
//		
////		double[] preproj = (double[]) cache.expectations.deepCopy();
...
////						ArrayMath.sum(((double[])lambda).value[i])
////						,ArrayMath.sum(((double[])preproj).value[i])));
...
////				ArrayMath.sum(((double[])lambda).value[max])
////				,ArrayMath.sum(((double[])preproj).value[max])));
//		
//
//	}
//
