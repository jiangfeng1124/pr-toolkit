...
public class L1Lmax implements CorpusConstraints {
	...
	class ChildParentIndex {public int type,i; public ChildParentIndex(int type2, int i2){type=type2;i=i2;}}
	class SentenceChildParent {public int s,c,p; public SentenceChildParent(int s2, int c2, int p2){s=s2;c=c2;p=p2;}}
	final ChildParentIndex[][][] scp2edge;  // indexed by sentence, child position, parent position
	final ChildParentIndex[][] sr2rooti; // idexed by sentence, root position; maps to root type, index 
	...
	public L1Lmax(Corpus corpus, ArrayList<DependencyInstance> toProject, PCType cType, PCType pType, boolean useRoot, boolean useDirection, double constraintStrength, boolean scaleByTagType, int minOccurrencesForProjection, String fileOfAllowedTypes) throws IOException{
		...
		// count how many edge types will not be projected for reportin
		int notToProject = 0;
		// create arrays..
		...
		scp2edge = new ChildParentIndex[toProject.size()][][];
		sr2rooti = new ChildParentIndex[toProject.size()][];
		...
		for (int s = 0; s < toProject.size(); s++) {
			...
			for (int childIndex = 0; childIndex < di.numWords; childIndex++) {
				...
				if (roottype >= 0) {
					...
					edge2scp[roottype][index] = new SentenceChildParent(s,childIndex,-1);
					indicesforcp.set(roottype, 1+index);
				}
				for (int parentIndex = 0; parentIndex < di.numWords; parentIndex++) {
					int edgetype = cstraints.edge2cid(di,childIndex, parentIndex);
					int index = indicesforcp.get(edgetype);
					edge2scp[edgetype][index] = new SentenceChildParent(s,childIndex,parentIndex);
	...
	public double getConstraintStrength(int edgeType){
		...
		// min occurrences for projection.. FIXME: this didn't work, and should be deleted
